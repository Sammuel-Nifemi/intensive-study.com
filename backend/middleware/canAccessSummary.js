  ...
  try {
    ...
    if (!selectedCount) {
      return res.status(403).json({
        message: "Select your courses before accessing summaries."
      });
    }
    ...
    if (freeCourseLimit <= 0) {
      return res.status(403).json({
        message: "Payment required to access summaries.",
        action: "PAY_2000"
      });
    }

    const usageCount = await getUsageCount(student._id, course._id, "summary");
    if (usageCount >= 1) {
      return res.status(403).json({
        message: "Summary limit reached for this course.",
        action: "PAY_2000"
      });
    }
