...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const filePath = material.filePath || material.fileUrl?.replace(/^\//, "");
    if (!filePath) {
      return res.status(404).json({ message: "Material file missing" });
    }

    res.download(path.resolve(filePath), material.title || undefined);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Failed to download materials" });
  }
});
